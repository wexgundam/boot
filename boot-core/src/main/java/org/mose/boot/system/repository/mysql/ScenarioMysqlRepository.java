...
public class ScenarioMysqlRepository extends AbstractStreamRepository<Scenario, Integer> implements IScenarioRepository {
    ...
    public Scenario queryOne(int id) {
        String sql = "select id, name, description, parent_id, url, url_target, icon, order_index, (select name from t_system_scenario where id=t.parent_id) parent_name from t_system_scenario t where id=?";
        return query().sql(sql).parameters(id).queryOne();
    }

    @Override
    public List<Scenario> queryAll() {
        String sql = "select id, name ,description, parent_id, url, url_target, icon, order_index from t_system_scenario";
    ...
    public int insertOne(Scenario scenario) {
        ...
        sql.append("insert into t_system_scenario(name, description, parent_id, url, url_target, icon, order_index)");
        sql.append(" values(:name, :description, :parentId, :url, :urlTarget, :icon, :orderIndex)");
    ...
    public int updateOne(Scenario scenario) {
        StringBuffer sql = new StringBuffer();
        sql.append("update t_system_scenario set name=:name, description=:description, parent_id=:parentId, url=:url, url_target=:urlTarget, icon=:icon, order_index=:orderIndex");
