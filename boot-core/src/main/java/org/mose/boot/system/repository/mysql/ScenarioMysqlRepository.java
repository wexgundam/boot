...
public class ScenarioMysqlRepository extends AbstractStreamRepository<Scenario, Integer> implements IScenarioRepository {
    ...
    public Scenario queryOne(int id) {
        String sql = "select id, name, description, parent_id, url, url_target, icon, order_index, (select name from t_scenario where id=t.parent_id) parent_name from t_scenario t where id=?";
        return query().sql(sql).parameters(id).queryOne();
    }

    @Override
    public List<Scenario> queryAll() {
        String sql = "select id, name ,description, parent_id, url, url_target, icon, order_index from t_scenario";
    ...
    public int insertOne(Scenario scenario) {
        ...
        sql.append("insert into t_scenario(name, description, parent_id, url, url_target, icon, order_index)");
    ...
    public int updateOne(Scenario scenario) {
        StringBuffer sql = new StringBuffer();
        sql.append("update t_scenario set name=:name, description=:description, parent_id=:parentId, url=:url, url_target=:urlTarget, icon=:icon, order_index=:orderIndex");
